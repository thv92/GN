...
module HeroScraper
    @@gf = GeneralFields
    def self.scrapeHero(linkToHero = nil)
        rawData = {}
        images = []
        file = File.open('./test.html')
        doc = Nokogiri::HTML(file)
        
        evoData = scrapeEvo(doc)
        ...
        rawData[@@gf::MD] = scrapeMetadata(doc)
        rawData[@@gf::STATS] = scrapeStats(doc)
        rawData[@@gf::ULT] = scrapeUltimate(doc)
        rawData[@@gf::ATTR] = scrapeAttributes(doc)
        rawData[@@gf::PASSIVES] = scrapePassives(doc)
        rawData[@@gf::EVO] = evoData[0]
        rawData[@@gf::IMGS] = [scrapeBannerImageURL(doc), *evoData[1]]
    ...
    def self.scrapeBannerImageURL(doc)
        #Banner Image URL
        bannerImageDict = {}
